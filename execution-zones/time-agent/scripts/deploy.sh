#!/bin/bash
#
# deploy.sh — Deploy Time Agent zone
#
# Usage:
...
log_info "Deploying Time Agent zone (env: ${DEPLOYMENT_ENV})"
...
if [[ "${FORCE_REBUILD}" == "true" ]]; then
    CONTEXT_ARGS="${CONTEXT_ARGS} --context forceTimeImageRebuild=$(date +%s)"
...
cd "${CDK_DIR}"
DEPLOYMENT_ENV="${DEPLOYMENT_ENV}" "${CDK_CLI}" deploy "${TIME_STACK}" \
