...
def get_thread_history(
    client: WebClient, channel: str, thread_ts: str, limit: int = 20
) -> List[Dict[str, Any]]:
    """
    ...
    Converse API format: content is an array of content blocks.
    ...
    Returns:
        ...
        [
            ...
            {"role": "assistant", "content": [{"text": "Bot response"}]},
    ...
    Example:
        ...
        >>> print(history)
        [{"role": "user", "content": [{"text": "Hello"}]}, {"role": "assistant", "content": [{"text": "Hi!"}]}]
    """
    try:
        ...
        # Convert Slack messages to Converse API format
        # Converse API requires content to be an array of content blocks
        history = []
        for msg in messages:
            ...
            if msg.get("bot_id") or msg.get("subtype") == "bot_message":
                ...
                history.append(
                    {
                        "role": "assistant",
                        "content": [{"text": text}],  # Converse API format
                    }
                )
            else:
                # This is a user message
                history.append(
                    {"role": "user", "content": [{"text": text}]}  # Converse API format
                )
        ...
        raise SlackApiError(
            f"Unexpected error retrieving thread history: {str(e)}", response={}
        )


def build_conversation_context(
    thread_history: List[Dict[str, str]], current_message: str
) -> List[Dict[str, str]]:
    """
    ...
    if current_message.strip():
        conversation.append({"role": "user", "content": current_message.strip()})
