...
describe("FileCreatorAgentStack", () => {
  let app: cdk.App;
  let stack: FileCreatorAgentStack;
  let template: Template;

  beforeEach(() => {
    app = new cdk.App();
    stack = new FileCreatorAgentStack(app, "TestFileCreatorAgentStack", {
  ...
  describe("A2A only (no legacy)", () => {
    ...
    it("must have output FileCreatorAgentRuntimeArn", () => {
      template.hasOutput("FileCreatorAgentRuntimeArn", {
        Description: Match.stringLikeRegexp("File Creator Agent.*Runtime ARN"),
      });
    });
  });

  describe("File Creator Agent AgentCore Runtime", () => {
    it("should create AgentCore Runtime with A2A protocol", () => {
      template.hasResourceProperties("AWS::BedrockAgentCore::Runtime", {
        AgentRuntimeName: Match.stringLikeRegexp("SlackAI_FileCreatorAgent"),
  ...
  describe("Cross-account support", () => {
    it("should create stack with verificationAccountId", () => {
      ...
      const crossAccountStack = new FileCreatorAgentStack(
        crossAccountApp,
        "CrossAccountFileCreatorAgentStack",
        {
          env: { account: "123456789012", region: "ap-northeast-1" },
      ...
      crossAccountTemplate.hasOutput("FileCreatorAgentRuntimeArn", {});
    });
  });
});
...
describe("cdk-nag security scan", () => {
  it("has no unresolved cdk-nag errors", () => {
    ...
    const nagStack = new FileCreatorAgentStack(nagApp, "NagTestStack", {
      env: { account: "123456789012", region: "ap-northeast-1" },
    });
