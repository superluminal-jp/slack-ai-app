#!/bin/bash
#
# deploy.sh — Deploy Web Fetch Agent zone
#
# Usage:
...
log_info "Deploying Web Fetch Agent zone (env: ${DEPLOYMENT_ENV})"
...
if [[ "${FORCE_REBUILD}" == "true" ]]; then
    CONTEXT_ARGS="${CONTEXT_ARGS} --context forceWebFetchImageRebuild=$(date +%s)"
...
cd "${CDK_DIR}"
DEPLOYMENT_ENV="${DEPLOYMENT_ENV}" "${CDK_CLI}" deploy "${WEB_FETCH_STACK}" \
