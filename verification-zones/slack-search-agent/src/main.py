"""
Slack Search Agent A2A Server — FastAPI on port 9000.

Receives JSON-RPC 2.0 payloads from Verification Agent and returns
Slack search/retrieval responses.
"""
...
def handle_message_tool(payload_json: str) -> str:
    """Process A2A message with Slack search tools and return formatted response JSON."""
    ...
    try:
        ...
        return json.dumps({
            ...
            "response_text": "エラーが発生しました。しばらくしてからお試しください。",
            "channel": channel,
            "correlation_id": correlation_id,
        })
...
def handle_invocation_body(body: bytes) -> dict:
    ...
    required = ("text", "channel", "bot_token")
    ...
    try:
        ...
        result_data = {"status": "error", "response_text": "Invalid response"}
