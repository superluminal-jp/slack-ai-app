/**
 * Slack Search Agent CDK Configuration Management
 *
 * @module verification-zones/slack-search-agent/cdk/lib/types/cdk-config
 */
...
export interface CdkConfig {
  ...
  slackSearchStackName: string;
  verificationAccountId: string;
  slackSearchAccountId: string;
  slackSearchAgentName?: string;
}
...
const CdkConfigSchema = z.object({
  ...
  slackSearchStackName: z.string().min(1, "slackSearchStackName is required"),
  verificationAccountId: z.string().regex(/^\d{12}$/, "verificationAccountId must be a 12-digit AWS account ID"),
  slackSearchAccountId: z.string().regex(/^\d{12}$/, "slackSearchAccountId must be a 12-digit AWS account ID"),
  slackSearchAgentName: z.string().regex(/^[a-zA-Z][a-zA-Z0-9_]{0,47}$/).optional(),
});
...
export function applyEnvOverrides(config: CdkConfig): CdkConfig {
  return {
    ...
    slackSearchAccountId: process.env.SLACK_SEARCH_ACCOUNT_ID || config.slackSearchAccountId,
  };
}
