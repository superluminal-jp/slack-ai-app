...
**目的**: AI プラットフォームとして AWS Bedrock Foundation Model を採用した決定と理由を記録する。
**対象読者**: 開発者、アーキテクト
**最終更新日**: 2026-02-14
...
- レイテンシはモデル、入力長、負荷状況に依存し、予測不可能（注: 設計時点での想定は5〜30秒程度）
...
- **Guardrails 統合**: Automated Reasoning（99%精度）で有害コンテンツ検出
...
- **レイテンシ**: モデル、入力長、負荷状況に依存し、予測不可能（注: 設計時点での想定は平均5〜30秒程度、リアルタイム会話には不向き）
...
- [実装詳細](../architecture.md) - Bedrock統合コード
- [セキュリティ実装](../security.md) - Guardrails実装
