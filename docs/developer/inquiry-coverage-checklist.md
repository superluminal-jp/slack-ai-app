...
| IP-010 | developer | セキュリティパイプラインの順序（存在確認・ホワイトリスト等）は？ | docs/developer/security.md | Covered | architecture の用語表と相互参照 |
...
**実施メモ（実装時）**: リリース前にレビュアーへ依頼し、結果を PR の Constitution Check またはここに 1 行で記載する。
