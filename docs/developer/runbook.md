...
5. [再デプロイと変更の戻し方（ポインタ）](#5-再デプロイと変更の戻し方ポインタ)
...
本システムに単一の「ワンクリックロールバック」ボタンはありません。運用上は次を想定します。

| 目的 | 次の一手 |
|------|----------|
| 直近のデプロイで不具合が出た | 問題のスタックを特定し、**前の Git コミットに戻したうえで** 該当ゾーンのみ `scripts/deploy.sh`（またはゾーンの `execution-zones/.../scripts/deploy.sh`）を再実行する。 |
| 設定だけ戻したい | `verification-zones/verification-agent/cdk/cdk.config.{env}.json` や Secrets の値を確認し、[クイックスタート](./quickstart.md) の前提に沿って修正する。 |
| エージェントが応答しない | [トラブルシューティング](./troubleshooting.md) の「返信なし診断」と CloudWatch ログを先に確認する。 |

詳細な切り分け手順は [トラブルシューティング](./troubleshooting.md) を参照してください。
