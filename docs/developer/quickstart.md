...
`DEPLOYMENT_ENV=dev ./scripts/deploy.sh deploy` は **実行ゾーン → 検証ゾーン** の順でスタックを進めます。既定の主な流れは次のとおりです（詳細は [初回デプロイ](#初回デプロイ) と [運用ガイド（Runbook）](./runbook.md)）。

1. **File Creator**（メインの Execution スタック）
2. **Docs**（プロジェクトドキュメント検索エージェント）
3. **Slack Search**（Slack 内検索エージェント）
4. **Verification**（検証ゾーン・Slack 連携の本体）
5. リソースポリシー・エージェントレディネス確認など（スクリプト内の後続フェーズ）

**既定で含まれないもの**: Time / Web Fetch は統合デプロイに含まれません。必要なら各ゾーンの `scripts/deploy.sh` を個別実行してください。
...
**Docs / Time Agent**（`docsAgentStackName` + `executionAccountId`、`timeAgentStackName` + `executionAccountId`）、**Fetch URL Agent**（`fetchUrlAgentStackName` + `webFetchAccountId`）、**Slack Search Agent**（`slackSearchStackName` + `slackSearchAccountId`）は、それぞれ同ディレクトリの `cdk.config.json.example` と `cdk/lib/types/cdk-config.ts` の必須フィールドと一致させてください。
...
このプロジェクトは **6 つの独立した CDK アプリ**（実行ゾーン 5 + 検証ゾーン 1: File Creator、Docs、Time、Slack Search、Web Fetch、Verification）を使用します。統合スクリプト `scripts/deploy.sh deploy` の**既定**では **Time** と **Web Fetch** はデプロイしません（実用・セキュリティ上の理由）。既定の順序は **File Creator → Docs → Slack Search → Verification**（リソースポリシーと検証は Phase 5〜6）です。Time / Web Fetch が必要な場合は後述のとおりゾーン単体の `scripts/deploy.sh` を実行してください。
...
**デプロイ順序**（`DEPLOYMENT_ENV=dev ./scripts/deploy.sh deploy` が自動で実行する順。手動で行う場合も File Creator / Docs → Slack Search → Verification が基本）:

1. File Creator → Docs → Slack Search の各スタックをデプロイし、Runtime ARN を取得（スクリプトが `verification-zones/verification-agent/cdk/cdk.config.{env}.json` の `executionAgentArns` 等を更新）。**Time / Web Fetch は既定では含まれません**（個別ゾーンの `scripts/deploy.sh` を参照）。
2. 検証ゾーン（Verification）をデプロイ

統合デプロイ完了後、agent registry から `time` と `fetch-url` の項目が削除されるため、これらを使う場合は Time / Web Fetch ゾーンをデプロイして再登録してください。
...
cd ../../docs-agent/cdk
npx cdk deploy SlackAI-DocsAgent-Dev --require-approval never

cd ../../time-agent/cdk
npx cdk deploy SlackAI-TimeAgent-Dev --require-approval never
...
npx cdk deploy SlackAI-FetchUrlAgent-Dev --require-approval never
...
- Time Agent（`SlackAI-TimeAgent-{Env}`）
- Docs Agent（`SlackAI-DocsAgent-{Env}`）
- Fetch URL Agent（`SlackAI-FetchUrlAgent-{Env}`）
- CloudWatch ログ・メトリクス
...
SlackAI-TimeAgent-Dev.TimeAgentRuntimeArn = arn:aws:bedrock-agentcore:...
SlackAI-DocsAgent-Dev.DocsAgentRuntimeArn = arn:aws:bedrock-agentcore:...
SlackAI-FetchUrlAgent-Dev.WebFetchAgentRuntimeArn = arn:aws:bedrock-agentcore:...
