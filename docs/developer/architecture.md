...
ドキュメント検索やオンボーディングで、次の言い回しが同じ概念を指すことがあります。

| よく使う語 | 同義・補足 |
|------------|------------|
| 検証ゾーン / Verification Zone | Slack 署名・Existence Check・**ホワイトリスト**・**レート制限**・重複排除を行う層。Verification Agent が動作する。 |
| 実行ゾーン / Execution Zone | **File Creator**、**Docs Agent**、**Slack Search Agent** など、Bedrock で処理する Execution Agent（A2A 被呼び出し側）。 |
| A2A | Agent-to-Agent。AgentCore Runtime 上で **JSON-RPC 2.0** によりエージェント同士を呼び出す方式。 |
| AgentCore | **Amazon Bedrock AgentCore**。コンテナ化されたエージェントのホスト。 |
| ホワイトリスト | 利用を許可するワークスペース・チャンネル・ユーザー（DynamoDB）。許可されないチャンネルでは応答しない。 |
| レート制限 | ユーザー単位の利用制限。超過時はしばらく待つか管理者に相談（ユーザー向け [FAQ](../user/faq.md) も参照）。 |
