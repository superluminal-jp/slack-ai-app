...
**定性的に押さえるポイント（説明用）**: コストはおおむね「**Bedrock のトークン量**」「**AgentCore 上のエージェント稼働**」「**Slack 連携の Lambda/DynamoDB の呼び出し回数**」の掛け合わせで増えます。利用者数を増やすほど Bedrock と AgentCore の比重が上がりやすい、と説明できます。
