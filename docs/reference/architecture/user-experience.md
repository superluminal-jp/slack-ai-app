...
**ステップ 1: ユーザーが @メンションを実行**

```
ユーザー: @AIアプリ名 AWS Bedrockの主な特徴は何ですか？
```

- ユーザーは Slack のメッセージ入力欄に `@AIアプリ名` メンションとリクエストを入力
...
**ステップ 3: バックグラウンド処理（Bedrock の処理時間に依存）**
...
- **処理時間**: Bedrock の処理時間はモデル、入力長、負荷状況に依存し、予測不可能

**ステップ 4: 最終レスポンスの表示（Bedrock の処理完了後）**
...
**ステップ 2: 自動一次回答の表示（Bedrock の処理完了後）**
...
| 最終レスポンス | Bedrock の処理完了後 | レスポンス確認 |
...
```
ユーザー: @AIアプリ名 [非常に長いリクエスト + 長いコンテキスト履歴]
[Bot] 処理中です... 少々お待ちください
[3秒後]
...
1. **待機時間**: 最終レスポンスは Bedrock の処理完了後に表示される（処理時間はモデル、入力長、負荷状況に依存し、予測不可能）
...
| Bedrock 処理時間      | モデル、入力長、負荷状況に依存（予測不可能） | CloudWatch Metrics   |
