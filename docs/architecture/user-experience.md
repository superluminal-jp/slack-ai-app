...
- バックグラウンドで以下が実行される:
  ...
  - Bedrock Guardrails がプロンプトインジェクションをチェック
  - 正規表現ベース PII 検出がレスポンスをフィルタリング
...
2. Guardrails 設定: プロンプトインジェクション検出の設定
...
**シナリオ 5: Existence Check エラー（偽造エンティティ ID）**
...
**シナリオ 6: Existence Check キャッシュヒット**
...
11. **セキュリティ**: PII 検出時は自動的にマスキングされ、警告が表示される
12. **コンテキスト履歴**: 前のリクエストを覚えており、コンテキストを維持
13. **多様な AI 機能**: 会話、画像生成、コード生成、データ分析などに対応
