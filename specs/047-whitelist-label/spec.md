...
### User Story 3 - 管理者が CDK config でチャンネルをラベル付きで定義する (Priority: P3)

`cdk.config.{env}.json` の `autoReplyChannelIds` / `mentionChannelIds` をオブジェクト形式（`{"id": "C0123456789", "label": "#general"}`）で記述できるようにし、従来の文字列リスト形式との後方互換を保つ。デプロイ時に CDK がラベルを Lambda 環境変数に渡すことで、インフラ側でもチャンネルの識別が容易になる。

**Why this priority**: CDK config はインフラ管理者が直接編集するファイルであり、ラベルがあればレビュー時のチャンネル特定が容易になる。ただし DynamoDB / Secrets Manager 経路より変更箇所が広いため P3。

**Independent Test**: `cdk.config.dev.json` にオブジェクト形式でチャンネルを設定し、CDK synth が成功してスタックに正しい設定が反映されることで独立して確認できる。

**Acceptance Scenarios**:

1. **Given** `cdk.config.dev.json` の `autoReplyChannelIds` が `[{"id": "C0123456789", "label": "#general"}]` 形式, **When** CDK デプロイが実行される, **Then** Lambda に `C0123456789` が渡され、デプロイが成功する
2. **Given** `cdk.config.dev.json` の `autoReplyChannelIds` が従来の `["C0123456789"]` 形式, **When** CDK デプロイが実行される, **Then** 後方互換が維持され、デプロイが成功する
3. **Given** CDK context で `--context autoReplyChannelIds=[{"id":"C0123456789","label":"#general"}]` を渡す, **When** CDK デプロイが実行される, **Then** 正しく解析されデプロイが成功する

---

### User Story 4 - 管理者が環境変数でチャンネルをラベル付きで設定する (Priority: P4)
...
**Why this priority**: 環境変数フォールバックはローカル開発や小規模デプロイで利用されるが、優先度は最も低い設定経路のため P4。
...
- **FR-004**: CDK config ファイル（`cdk.config.{env}.json`）の `autoReplyChannelIds` および `mentionChannelIds` で、各要素をオブジェクト形式（`{"id": "...", "label": "..."}`）と従来の文字列形式の両方で受け付けなければならない
- **FR-005**: CDK context パラメータ（`--context autoReplyChannelIds=...`）でも同様にオブジェクト形式の JSON 配列を受け付けなければならない
- **FR-006**: 環境変数 `WHITELIST_CHANNEL_IDS` で `ID:ラベル` 形式と従来の文字列形式の両方を受け付けなければならない
- **FR-007**: 認証判定は `entity_id`（チャンネルID文字列）の一致のみで行い、`label` の値は判定に影響してはならない
- **FR-008**: 認証成功・失敗のログに `label` が設定されている場合はその値を含めなければならない
- **FR-009**: `label` が未設定のエントリは既存の動作と完全に後方互換でなければならない
- **FR-010**: `team_id` および `user_id` の各エントリは本変更の対象外とする（`channel_id` エントリのみ `label` をサポート）
...
- **SC-004**: DynamoDB・Secrets Manager・CDK config・環境変数の全 4 設定経路で `label` が正しく読み込まれることがテストで確認できる
...
- CDK config の型定義（TypeScript）も `string | {id: string; label: string}` のユニオン型に変更する
